...
export function Skateboard({
  ...
  const gripTapeURLs: string[] = griptapeTextureURLs ?? [];
  const gripTapeDiffuse = useTexture(gripTapeURLs) as THREE.Texture[];

  gripTapeDiffuse.forEach((texture: THREE.Texture) => {
    texture.flipY = false;
    texture.colorSpace = THREE.SRGBColorSpace;
  });

  const gripTapeTextureIndex = gripTapeURLs.findIndex(
    (url) => url === griptapeTextureURL
  );
  const gripTapeTexture = gripTapeDiffuse[gripTapeTextureIndex];
  ...
  const gripTapeMaterial = useMemo(() => {
    ...
    return material;
  }, [gripTapeTexture, gripTapeRoughness]);
  ...
  const positions = useMemo(
    () =>
      ({
        ...
        vertical: {
          rotation: [0.5 * Math.PI, 0, 1] as const,
